# Ensemble Extension 1: Hard Voting Ensemble of BERT, BERT + CNN, BERT + LSTM

## Overview
This extension implements a **Hard Voting ensemble classifier** that combines the predictions of three independently trained models: (1) a fine-tuned BERT baseline, (2) BERT + CNN, and (3) BERT + LSTM. Each model produces a binary prediction (hate vs. non-hate) for a given input post, and the final ensemble prediction is determined by **majority vote** across the three models.
...
├── src/
│   ├── Hard_Voting.py # Script for Hard Voting ensemble
```

Running the Script

To execute the Hard Voting Ensemble, execute the following command from within the src folder
```python
python Hard_Voting.py
```

At the end of execution, you will see the following three files in your directory
- Hard-Voting-train-results.csv: List of evaluation metrics on Training Dataset
- Hard-Voting-dev-results.csv: List of evaluation metrics on Validation/Dev Dataset
- Hard-Voting-test-results.csv: List of evaluation metrics on Test Dataset
