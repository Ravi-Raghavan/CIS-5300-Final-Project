...
> **Note to Grader:** We used the publicly available dataset, which includes 1,101,165 social media posts, before sampling and splitting the data into train/dev/test sets. Refer to the Data Splits section to see how we sampled the data and split into train/dev/test sets. 
...
> **Note to Grader:** Due to the large size of the full dataset, we first sampled **3% of the original dataset using a stratified sampling strategy by label**, and then performed a **stratified train/dev/test split by label** on this subset.

We ensured that the **same post does not appear in multiple splits** by using `sklearn`’s `train_test_split` with label stratification. This was straightforward since each post in the original dataset is unique.
...
- **Training set:** (26427 posts)  
- **Development set (dev):**  (3303 posts)  
- **Test set:** (3304 posts)